...
public class PredefinedStepDefinitions implements BaseTest {
    ...
    //Step to navigate to specified URL
    @Then("^I Tab from type (.+) \"(.*?)\"$")
    public void i_tab(String type, String accessName) throws Exception
    {
        //miscmethodObj.validateLocator(type);
        navigationObj.tab(type, accessName);
    ...
    //step to assert checkbox is checked or unchecked
    @Then("^checkbox having (.+) \"(.*?)\" will be (checked|unchecked)$")
    public void checkbox_checked(String type, String accessName,String state) throws Exception
    {
        miscmethodObj.validateLocator(type);
        boolean flag = state.equals("checked");
        clickObj.clickCheckBox(type,accessName,flag);
        //assertionObj.isCheckboxChecked(type, accessName, flag);
    }
